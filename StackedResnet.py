...
class AdaptaBlock(nn.Module):
    def __init__(self, UPCONV=False):
        ...
        self.conv_transpose00 = nn.ConvTranspose2d(in_channels=512, out_channels=512, kernel_size=3, stride=2, padding=1) # 23, 31
        self.bnt00 = nn.BatchNorm2d(512)
        self.conv_transpose01 = nn.ConvTranspose2d(in_channels=256, out_channels=256, kernel_size=3, stride=2, padding=1) # 45, 61
        self.bnt01 = nn.BatchNorm2d(256)
        self.conv_transpose02 = nn.ConvTranspose2d(in_channels=128, out_channels=128, kernel_size=3, stride=2, padding=1) # 89, 121
        self.bnt02 = nn.BatchNorm2d(128)
        self.conv_transpose03 = nn.ConvTranspose2d(in_channels=64, out_channels=64, kernel_size=3, stride=2, padding=1) # 177, 241
        self.bnt03 = nn.BatchNorm2d(64)
        ...
        self.conv_transpose10 = nn.ConvTranspose2d(in_channels=256, out_channels=256, kernel_size=3, stride=2, padding=1) # 47, 61
        self.bnt10 = nn.BatchNorm2d(256)
        self.conv_transpose11 = nn.ConvTranspose2d(in_channels=128, out_channels=128, kernel_size=3, stride=2, padding=1) # 93, 121
        self.bnt11 = nn.BatchNorm2d(128)
        self.conv_transpose12 = nn.ConvTranspose2d(in_channels=64, out_channels=64, kernel_size=3, stride=2, padding=1) # 185, 241
        self.bnt12 = nn.BatchNorm2d(64)
        ...
        self.conv_transpose20 = nn.ConvTranspose2d(in_channels=128, out_channels=128, kernel_size=3, stride=2, padding=1) # 93, 123
        self.bnt20 = nn.BatchNorm2d(128)
        self.conv_transpose21 = nn.ConvTranspose2d(in_channels=64, out_channels=64, kernel_size=3, stride=2, padding=1) # 185, 245
        self.bnt21 = nn.BatchNorm2d(64)
        
        self.relu = nn.ReLU()

    def forward (self, tensor0, tensor1, tensor2):
        # ORIGINAL: downlsampling and upsampling
        """
        # 8, 512 , 12, 16   
        ...
        """
        # MODIFIED: upsampling and downsampling
        
        # 8, 512 , 12, 16   
        x = self.bnt00(self.conv_transpose00(tensor0))
        x = self.relu(self.bn00(self.conv00(x)))
        x = self.bnt01(self.conv_transpose01(x))
        x = self.relu(self.bn01(self.conv01(x)))
        x = self.bnt02(self.conv_transpose02(x))
        x = self.relu(self.bn02(self.conv02(x)))
        ...
        # 8, 256 , 24, 31
        x = self.bnt10(self.conv_transpose10(tensor1))
        ...
        # 8 , 128 , 47, 62 
        x = self.bnt20(self.conv_transpose20(tensor2))
