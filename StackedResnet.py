...
class ViTForecaster (nn.Module):
    def __init__(
        ...
        if (self.dim == '2D_ViT_im'):
        ...
        elif (self.dim == '2D_ViT_SR_feat_in_puzzle'):
            self.stacked_resnet = stacked_resnet
            self.stacked_resnet.resnet.fc = nn.Identity()
            ...
            if (self.UPCONV):
                ...
                self.conv_transpose00 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) # 23, 31
                self.bnt00 = nn.BatchNorm2d(1)
                self.conv_transpose01 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) # 45, 61
                self.bnt01 = nn.BatchNorm2d(1)
                self.conv_transpose02 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) # 89, 121
                self.bnt02 = nn.BatchNorm2d(1)
                self.conv_transpose03 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) # 177, 241
                self.bnt03 = nn.BatchNorm2d(1)
            ...
            if (self.UPCONV):
                self.conv_transpose10 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) # 47, 61
                self.bnt10 = nn.BatchNorm2d(1)
                self.conv_transpose11 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) # 93, 121
                self.bnt11 = nn.BatchNorm2d(1)
                self.conv_transpose12 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) # 185, 241
                self.bnt12 = nn.BatchNorm2d(1)
            ...
            if (self.UPCONV):
                self.conv_transpose20 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) # 93, 123
                self.bnt20 = nn.BatchNorm2d(1)
                self.conv_transpose21 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) # 185, 245
                self.bnt21 = nn.BatchNorm2d(1)
    ...
    def forward(self, x_img):
        ...
        if (self.dim == '2D_ViT_im'):
            ...
            #x_feat = get_interested_feature_map(self.stacked_resnet, x_img, list(self.stacked_resnet.resnet.children())[-4][-1].conv2)
            tensor1 = get_interested_feature_map(self.stacked_resnet, x_img, list(self.stacked_resnet.resnet.children())[-4][-1].conv1) 
            tensor2 = get_interested_feature_map(self.stacked_resnet, x_img, list(self.stacked_resnet.resnet.children())[-4][-1].conv2) 
            tensor3 = get_interested_feature_map(self.stacked_resnet, x_img, list(self.stacked_resnet.resnet.children())[-4][-2].conv1)
            tensor4 = get_interested_feature_map(self.stacked_resnet, x_img, list(self.stacked_resnet.resnet.children())[-4][-2].conv2)
            ...
            x_feat = F.interpolate(merged_tensor, size=(224, 224), mode='bilinear', align_corners=False)
            x_feat = self.conv1x1_1(x_feat)
            x_feat = self.bn1(x_feat)
            x_feat = self.conv1x1_2(x_feat)
            x_feat = self.bn2(x_feat)
            x_feat = self.conv1x1_3(x_feat)
            x_feat = self.bn3(x_feat)
            x_vit = self.ViT(x_feat).logits
            ...
            x_feat_0 = self.conv00(tensor0)
            x_feat_0 = self.bn00(x_feat_0)
            x_feat_0 = self.conv01(x_feat_0)
            x_feat_0 = self.bn01(x_feat_0)
            x_feat_0 = self.conv02(x_feat_0)
            x_feat_0 = self.bn02(x_feat_0)
            x_feat_0 = self.conv03(x_feat_0)
            x_feat_0 = self.bn03(x_feat_0)
            x_feat_0 = self.conv04(x_feat_0)
            x_feat_0 = self.bn04(x_feat_0) # 8, 1 , 12, 16
            if (self.UPCONV):
                x_feat_0 = self.conv_transpose00(x_feat_0)
                x_feat_0 = self.bnt00(x_feat_0)
                x_feat_0 = self.conv_transpose01(x_feat_0)
                x_feat_0 = self.bnt01(x_feat_0)
                x_feat_0 = self.conv_transpose02(x_feat_0)
                x_feat_0 = self.bnt02(x_feat_0)
                x_feat_0 = self.conv_transpose03(x_feat_0)   
                x_feat_0 = self.bnt03(x_feat_0)
            x_feat_0 = F.interpolate(x_feat_0, size=(224, 224), mode='bilinear', align_corners=False) # 8, 1, 224, 224
                
            x_feat_1 = self.conv10(tensor1)
            x_feat_1 = self.bn10(x_feat_1)
            x_feat_1 = self.conv11(x_feat_1)
            x_feat_1 = self.bn11(x_feat_1)
            x_feat_1 = self.conv12(x_feat_1)
            x_feat_1 = self.bn12(x_feat_1)
            x_feat_1 = self.conv13(x_feat_1)
            x_feat_1 = self.bn13(x_feat_1) # 8, 1 , 24, 31
            if (self.UPCONV):
                x_feat_1 = self.conv_transpose10(x_feat_1)
                x_feat_1 = self.bnt10(x_feat_1)
                x_feat_1 = self.conv_transpose11(x_feat_1)
                x_feat_1 = self.bnt11(x_feat_1)
                x_feat_1 = self.conv_transpose12(x_feat_1)
                x_feat_1 = self.bnt12(x_feat_1)
            x_feat_1 = F.interpolate(x_feat_1, size=(224, 224), mode='bilinear', align_corners=False) # 8, 1, 224, 224
            
            x_feat_2 = self.conv20(tensor2)
            x_feat_2 = self.bn20(x_feat_2)
            x_feat_2 = self.conv21(x_feat_2)
            x_feat_2 = self.bn21(x_feat_2)
            x_feat_2 = self.conv22(x_feat_2)
            x_feat_2 = self.bn22(x_feat_2) # 8 , 1 , 47, 62 
            if (self.UPCONV):
                x_feat_2 = self.conv_transpose20(x_feat_2)
                x_feat_2 = self.bnt20(x_feat_2)
                x_feat_2 = self.conv_transpose21(x_feat_2)
                x_feat_2 = self.bnt21(x_feat_2)
            x_feat_2 = F.interpolate(x_feat_2, size=(224, 224), mode='bilinear', align_corners=False) # 8, 1, 224, 224
            
            x_feat_merged = torch.cat((x_feat_0,x_feat_1, x_feat_2), axis=1)
